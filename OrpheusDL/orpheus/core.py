...
class Orpheus:
    def __init__(self, private_mode=False, module_options=None):
        ...
        # Get the absolute path of the OrpheusDL directory
        orpheus_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

        self.data_folder_base = os.path.join(orpheus_dir, 'config')
        ...
        extensions_dir = os.path.join(orpheus_dir, 'extensions')
        os.makedirs(extensions_dir, exist_ok=True)
        for extension in os.listdir(extensions_dir):  # Loading extensions
            if os.path.isdir(os.path.join(extensions_dir, extension)) and os.path.exists(os.path.join(extensions_dir, extension, 'interface.py')):
                class_ = getattr(importlib.import_module(f'OrpheusDL.extensions.{extension}.interface'), 'OrpheusExtension', None)
                if class_:
        ...
        modules_dir = os.path.join(orpheus_dir, 'modules')
        os.makedirs(modules_dir, exist_ok=True)
        module_list = [module.lower() for module in os.listdir(modules_dir) if os.path.exists(os.path.join(modules_dir, module, 'interface.py'))]
        ...
        for module in module_list:  # Loading module information into module_settings
            module_information: ModuleInformation = getattr(importlib.import_module(f'OrpheusDL.modules.{module}.interface'), 'module_information', None)
        ...
        self.update_module_storage(module_options)

        # Manually update settings for modules passed in module_options
        if module_options:
            for module_name, options in module_options.items():
                if module_name in self.settings['modules']:
                    self.settings['modules'][module_name].update(options)
                else:
                    self.settings['modules'][module_name] = options

        for i in self.extension_list:
            extension_settings: ExtensionInformation = getattr(importlib.import_module(f'OrpheusDL.extensions.{i}.interface'), 'extension_settings', None)
    ...
    def load_module(self, module: str):
        ...
        if module not in self.loaded_modules:
            class_ = getattr(importlib.import_module(f'OrpheusDL.modules.{module}.interface'), 'ModuleInterface', None)
            if class_:
                ...
                if self.module_settings[module].login_behaviour is ManualEnum.orpheus:
                    ...
                    if temporary_session and temporary_session['clear_session'] and not self.settings['global']['advanced']['advanced_login_system']:
                        ...
                        if not temporary_session.get('hashes') or \
                            ...
                            try:
                                username = settings.get('username') or settings.get('email')
                                password = settings.get('password')
                                if username and password:
                                    loaded_module.login(username, password)
                                else:
                                    print(f"Username or password not found for {module}")
                            except Exception as e:
                                print(f"Failed to log in to {module}: {e}")
    ...
    def update_module_storage(self, module_options=None): # Should be refactored eventually
        if module_options is None:
            module_options = {}
        ## Settings
        ...
        # Merge module_options into new_settings
        for module, options in module_options.items():
            if module in new_settings['modules']:
                new_settings['modules'][module].update(options)
            else:
                new_settings['modules'][module] = options
...
def orpheus_core_download(orpheus_session: Orpheus, media_to_download, third_party_modules, separate_download_module, output_path, progress_callback=None):
    ...
    for mainmodule, items in media_to_download.items():
        for media in items:
            ...
            if separate_download_module and separate_download_module != 'default' and separate_download_module != mainmodule:
                ...
                if mediatype is DownloadTypeEnum.album:
                    downloader.download_album(media_id, extra_kwargs=media.extra_kwargs) # Progress callback not supported for albums yet
                elif mediatype is DownloadTypeEnum.track:
                    downloader.download_track(media_id, extra_kwargs=media.extra_kwargs, progress_callback=progress_callback)

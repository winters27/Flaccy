...
class ModuleInterface:
    ...
    def search(self, query_type: DownloadTypeEnum, query, track_info: TrackInfo = None, limit: int = 10, offset: int = 0):
        results = {}
        if track_info and track_info.tags.isrc:
            results = self.session.search(query_type.name, track_info.tags.isrc, limit, offset)
        if not results:
            results = self.session.search(query_type.name, query, limit, offset)

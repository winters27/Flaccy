...
class Qobuz:
    ...
    def _get(self, url: str, params=None):
        ...
        r = self.s.get(f'{self.api_base}{url}', params=params, headers=self.headers(), timeout=15)
    ...
    def search(self, query_type: str, query: str, limit: int = 10, offset: int = 0):
        params = {
            'query': query,
            'type': query_type + 's',
            'limit': str(limit),
        ...
        signature = self.create_signature('catalog/search', params)
        params['request_ts'] = signature[0]
        params['request_sig'] = signature[1]

        return self._get('catalog/search', params)

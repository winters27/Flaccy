...
from OrpheusDL.utils.utils import sanitise_name, silentremove, download_to_temp, create_temp_filename, create_requests_session
...
class ModuleInterface:
    ...
    def __init__(self, module_controller: ModuleController):
        ...
        while True:
            ...
            if not saved_sessions:
                # Force TV login for non-interactive use
            ...
            for session_type in self.available_sessions:
                ...
                if not subscription:
                    # For non-interactive use, just exit the loop and try to continue
                    self.print(f'{module_information.service_name}: Subscription check failed, attempting to continue...')
                    break
    ...
    def auth_session(self, session, session_type, login_session):
        if login_session:
            ...
            # Force TV auth for non-interactive use
            self.print(f'{module_information.service_name}: Creating a TV session')
            session.auth()
    ...
    def search(self, query_type: DownloadTypeEnum, query: str, track_info: TrackInfo = None, limit: int = 20, offset: int = 0):
        if track_info and track_info.tags.isrc:
            results = self.session.get_tracks_by_isrc(track_info.tags.isrc)
        else:
            results = self.session.get_search_data(query, limit=limit, offset=offset)[query_type.name + 's']
